    ...
    public function createThumbs()
    {
        ...
        foreach ($this->thumbs as $profile => $config) {
            ...
            if (is_file($path) && !is_file($thumbPath)) {
                ...
                \Yii::warning([$path, $config],__METHOD__);
                if($config === null){
                    $config = [];
                }
                ...
                if($size > 2000000){
                    $quality = round(2000000/$size*100); //Высчитваем на сколько процентов надо снизить качество изображения, чтобы быть в пределах 2 Мб
                    $width=$thumb->getMaxWidth()/$quality;
                    $height=$thumb->getMaxHeight()/$quality;
