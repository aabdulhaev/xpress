    ...
    <tr>
        <td align="left" valign="middle" style="padding-top:16px;">
            <span class="text" style="font-size:16px;color:#546B82;">
                <?= $sender->getRoleLabel() ?> <?= Html::encode($sender->fullName) ?> оставил комментарии о сессии
            с <?= ArrayHelper::getValue($roles, $recipient->role) ?> <?= Html::encode($recipient->fullName) ?> <?= Yii::$app->formatter->asDatetime($session->start_at_tc, 'php:d-m-Y H:i') ?> (GMT +3)
	        </span>
        </td>
    </tr>
